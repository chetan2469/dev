...
class _Admin extends State<Admin> {
  ...
  List<Record> Admin = new List();
  List<Record> filteredAdmin = new List();
  ...
  String getSortByChip(int index) {
    if (searchBy == 'SAK-ID') {
      return filteredAdmin[index].sakId.toString();
    } else if (searchBy == 'Name') {
      return filteredAdmin[index].name.length > 12
          ? filteredAdmin[index].name.substring(0, 10) + '...'
          : filteredAdmin[index].name;
    } else if (searchBy == 'City') {
      return filteredAdmin[index].birthPlace.length > 12
          ? filteredAdmin[index].birthPlace.substring(0, 10) + '...'
          : filteredAdmin[index].birthPlace;
    } else if (searchBy == 'Age') {
      return filteredAdmin[index].dob.toDate().day.toString() +
          ' / ' +
          filteredAdmin[index].dob.toDate().month.toString() +
          ' / ' +
          filteredAdmin[index].dob.toDate().year.toString();
    }
  }
                            ...
                            onChanged: (String searchKey) {
                              debouncer.run(() {
                                setState(() {
                                  ...
                                  filteredAdmin = Admin.where((str) => (str.name
                                          .toLowerCase()
                                          .contains(searchKey.toLowerCase()) ||
                                      str.name.toLowerCase().contains(
                                          searchKey.toLowerCase()))).toList();
                                });
                              });
                            },
                            style: TextStyle(
                      ...
                      return InkWell(
                        onTap: () {
                          Navigator.push(
                              ...
                              MaterialPageRoute(
                                  builder: (context) =>
                                      CandidateShowDetails(filteredAdmin[i])));
                        },
                        child: Container(
                                ...
                                decoration: BoxDecoration(
                                    image: DecorationImage(
                                      ...
                                      image: CachedNetworkImageProvider(
                                          filteredAdmin[i].thumbnail != null
                                              ? filteredAdmin[i].thumbnail
  ...
  fetchCandidateData() async {
    ...
    documents.forEach((data) {
      ...
      Admin.add(record);
      filteredAdmin.add(record);
      // Firestore.instance
      //     .collection('SAK')
      //     .document(record.email)
      //     .updateData({'selfDestroyed': false});
    });
