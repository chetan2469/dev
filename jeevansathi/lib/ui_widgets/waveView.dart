...
import 'package:jeevansathi/ui_widgets/appTheme.dart';
import 'package:vector_math/vector_math.dart' as vector;

class WaveView extends StatefulWidget {
  final double percentageValue;
  final Color liquidColor, liquidUpperLayerColor, tubeFontColor;

  const WaveView(
      {Key key,
      this.percentageValue = 100.0,
      this.liquidColor = AppTheme.nearlyDarkBlue,
      this.tubeFontColor = Colors.white,
      this.liquidUpperLayerColor = Colors.blueAccent})
      : super(key: key);
  @override
...
class _WaveViewState extends State<WaveView> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    waveAnimationController.addListener(() {
      animList1.clear();
      for (int i = -2 - bottleOffset1.dx.toInt();
      ...
      animList2.clear();
      for (int i = -2 - bottleOffset2.dx.toInt();
          i <= MediaQuery.of(context).size.width;
          i++) {
        animList2.add(
          new Offset(
            ...
            new ClipPath(
              child: new Container(
                decoration: BoxDecoration(
                  ...
                  gradient: LinearGradient(
                    colors: [
                      widget.liquidColor.withOpacity(0.4),
                      widget.liquidColor
                    ],
                    begin: Alignment.topLeft,
                    end: Alignment.bottomRight,
